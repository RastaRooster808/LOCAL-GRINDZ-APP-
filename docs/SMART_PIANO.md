...
The four velocity layers are rendered by additive synthesis at start-up — this
repository ships no sample library. To play actual recordings instead, point the
**Instrument** panel at a manifest: each note then switches to its recording the
first time you play it, one file at a time, with synthesis covering the first
strike and anything a library doesn't reach.

`public/piano/fluidr3.manifest.json` works with nothing to download or host.
Start-to-finish instructions, the manifest format, the level-matching rules and
troubleshooting are in **`docs/SAMPLE_LIBRARY.md`**.
