...
export function Account() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_e, session) => {
      setUser(session?.user ?? null);
      if (session?.user?.email) loadOrders(session.user.email);
      else { setLoading(false); setOrders([]); setPoints(0); }
    });

    return () => subscription.unsubscribe();
  }, []);

  async function loadOrders(customerEmail: string) {
    ...
    setOrders(list);
    // $1 spent = 10 points, completed orders only
    setPoints(
              ...
              {activeOrders.map(o => {
                ...
                return (
                  <Link key={o.id} to={`/order/${o.id}`} className="account-order-card account-order-card--active">
                    <div className="order-card-vendor">{vendorName}</div>
                    <div className={`order-card-status-pill status-${o.status}`}>
                      {STATUS_LABELS[o.status]}
                    </div>
                    <div className="order-card-total">${Number(o.total).toFixed(2)}</div>
